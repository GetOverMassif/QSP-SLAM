...
int main(int argc,char* argv[])
{
    ...
    // auto iter = objObs.begin(); 
    // iter++; 
    // iter++;
    
    // ORIGIN: 从文件读取
    // std::map<double, int> mapFrameObj = (iter)->second; // 取出第一个

    // DEBUG: 单独指定测试
    // std::map<double, int> mapFrameObj = LoadFrameObjSettingsStatic();
    ...
    std::map<int, g2o::ellipsoid> mapInsEllipsoid; // 存储多帧优化结果
    std::map<int, g2o::ellipsoid> mapInsEllipsoidTexture; // 存储多帧优化结果
    std::map<int, g2o::ellipsoid> mapInsEllipsoidSVD, mapInsEllipsoidQuadricSLAM; // 存储多帧优化结果
    ...
    for(auto& pair : objObs){
        ...
        // 文字输出所有数据, Check IO.cpp
        // OutPutFrameData(frameDatas);

        // 可视化各个数据
        if(DebugMode){
            OutputObjectObservation(mapFrameObj);
            VisualizeFrameData(frameDatas, pSLAM);
            std::cout << "Visualizing frame data ... " << std::endl;
            std::cout << std::endl << std::endl << std::endl;
        }

        // Exp1 : QuadrcSLAM & SVD
        g2o::ellipsoid e_svd, e_quadricslam;
        ExperiementQuadricSLAM(frameDatas, calib, rows, cols, e_svd, e_quadricslam);

        // Exp2 : Ground Init & Pri & Texture 
        g2o::ellipsoid e_init, e_opt, e_opt_ground, e_opt_ground_pri, e_opt_texture;
        std::vector<Frame *> pFrames;
        Measurements mms;
        Objects objs;
        ExperiementStoSLAM(frameDatas, calib, rows, cols, 
            e_init, e_opt, e_opt_ground, e_opt_ground_pri, e_opt_texture,
            pFrames, mms, objs);

        // Visualize Result
        VisualizeExperienmentResult(pSLAM, frameDatas, calib, rows, cols,
            pFrames, mms, objs,
            e_svd, e_quadricslam, 
            e_init, e_opt, e_opt_ground, e_opt_ground_pri, e_opt_texture);

        // 接下来保存本物体的关键信息
        // 1) 下面每个观测的初始化物体 [另一个函数已经可以输出保存了]
        // 2) 联合优化得到的物体
        // 3) 结合纹理之后得到的物体 [纹理的先不考虑]
        mapInsEllipsoid.insert(make_pair(instance, e_opt_ground_pri));
        mapInsEllipsoidTexture.insert(make_pair(instance, e_opt_texture));
        mapInsEllipsoidSVD.insert(make_pair(instance, e_svd));
        mapInsEllipsoidQuadricSLAM.insert(make_pair(instance, e_quadricslam));

        std::cout << std::endl << "End of this object." << std::endl;
        // cv::waitKey();
    }


    // 完成所有物体的计算，输出保存
    SaveEllipsoidsToFile(mapInsEllipsoid, "./result_objects_optimization.txt");
    SaveEllipsoidsToFile(mapInsEllipsoidTexture, "./result_objects_optimization_texture.txt");
    SaveEllipsoidsToFile(mapInsEllipsoidSVD, "./result_objects_SVD.txt");
    SaveEllipsoidsToFile(mapInsEllipsoidQuadricSLAM, "./result_objects_QuadricSLAM.txt");
