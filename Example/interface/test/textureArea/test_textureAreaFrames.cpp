...
int main(int argc,char* argv[])
{
    ...
    std::vector<Frame *> vecFrames = GenerateFrames(frameDatas);
    ...
    for(auto pFrame:vecFrames )
    {
        ...
        g2o::ellipsoid* pObjw = new g2o::ellipsoid(InitializeQuadrics(frameDatas, pTracker->mCalib, pTracker->mRows, pTracker->mCols));        

        if(!pObjw) continue;
        
        auto bbox = pObjw->bbox;  // 如何获得bbox?????@@@@
        auto e_local = pObjw->transform_from(pFrame->cam_pose_Tcw);

        // 绘制bbox
        cv::Mat rgb = pFrame->rgb_img;
        cv::Mat rgbShow = pDrawer->drawFrameOnImage(rgb);
        ...
        // 尝试构建小优化，并将结果做比较
        g2o::ellipsoid* pObjw_rotOpt = new g2o::ellipsoid(opt.optimizeWithTexture(e_local, bbox, pTracker->mCalib, rgb));
        rgbShow = opt.VisualizeResultOnImage(rgb, e_local, bbox, pTracker->mCalib);
        ...
        while(1)
        {
            ...
            g2o::ellipsoid eRot = e_local.rotate_ellipsoid(theta);
            ...
            rgbShow = opt.VisualizeResultOnImage(rgb, eRot, bbox, pTracker->mCalib);
            cv::imshow("rgbShow", rgbShow);
