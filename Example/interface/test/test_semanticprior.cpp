...
int main(int argc,char* argv[]) {
    ...
    // 手动设置nmb 第二轮实验的地平面
    SLAM.getTracker()->SetGroundPlaneMannually(Vector4d(0.00667181,  -0.015002,   0.999865,  0.0274608));
    ...
    double dPriorDLast = 0, dPriorELast = 0, dWeightLast = 0, dGroundWeightLast = 0;
    double dPriorDCurrent, dPriorECurrent, dWeightCurrent, dGroundWeightCurrent;

    pViewer->addDoubleMenu("----Semantic Prior Debug---",0,1,0);
    int iMenuPriorD = pViewer->addDoubleMenu("Prior_d",1,100,1);
    int iMenuPriorE = pViewer->addDoubleMenu("Prior_e",1,100,1);
    int iMenuWeight = pViewer->addDoubleMenu("Weight",0,100,1);
    int iMenuGroundWeight = pViewer->addDoubleMenu("GroundWeight",0,100,1);
    ...
    while(1)
    {
        ...
        // 判断动态参数.
        pViewer->getValueDoubleMenu(iMenuPriorD, dPriorDCurrent);
        pViewer->getValueDoubleMenu(iMenuPriorE, dPriorECurrent);
        pViewer->getValueDoubleMenu(iMenuWeight, dWeightCurrent);
        pViewer->getValueDoubleMenu(iMenuGroundWeight, dGroundWeightCurrent);

        if( diff(dPriorDLast, dPriorDCurrent) || diff(dPriorELast, dPriorECurrent) || diff(dWeightLast, dWeightCurrent) || diff(dGroundWeightLast, dGroundWeightCurrent) )
        {
            Config::SetValue<double>("SemanticPrior.PriorD", dPriorDCurrent);    
            Config::SetValue<double>("SemanticPrior.PriorE", dPriorECurrent); 
            Config::SetValue<double>("SemanticPrior.Weight", dWeightCurrent); 
            Config::SetValue<double>("SemanticPrior.GroundWeight", dGroundWeightCurrent);
            
            dPriorDLast = dPriorDCurrent;
            dPriorELast = dPriorECurrent;
            dWeightLast = dWeightCurrent;
            dGroundWeightLast = dGroundWeightCurrent;
            bRun = true;   
        }

        if(bRun){
            std::cout << "[ Param ]" << std::endl;
            std::cout << " - dPriorDCurrent : " << dPriorDLast << std::endl;
            std::cout << " - dPriorECurrent : " << dPriorELast << std::endl;
            std::cout << " - dWeightCurrent : " << dWeightLast << std::endl;
