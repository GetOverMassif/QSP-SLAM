...
import subprocess
from extract_map_objects_multi import read_yaml_file
...
if __name__ == "__main__":
    ...
    config_file = yaml_data['DetectorConfigPath']

    dataset_path = yaml_data['Dataset.Path.Root']

    # decoder_paths = yaml_data['DecoderPaths']

    if not osp.exists(objects_dir):
        print(os.getcwd())
        cmd = f"python /home/lj/Documents/QSP-SLAM/extract_map_objects_multi.py -m {args.map_dir} -y{args.yaml}"
    ...
    map_pcd_file = osp.join(dataset_path, 'result/map.pcd')
    if osp.exists(map_pcd_file):
        map_point_cloud = o3d.io.read_point_cloud(map_pcd_file)
